...
class Test_260_UpdatingItemTodoPriorities: XCTestCase {
    ...
    func test_000_anItemsPriorityInTheWaitingListCanBeIncreasedByDraggingItUpInTheList() throws {
        ...
        let firstItemTitle = try app.contentRowTextFieldValue(0)
        let secondItemTitle = try app.contentRowTextFieldValue(1)
        ...
        XCTAssertEqual(try app.contentRowTextFieldValue(1), firstItemTitle,
    ...
    func test_010_anItemsPriorityInTheWaitingListCanBeDecreasedByDraggingItDownTheList() throws {
        ...
        let penultimateTitle = try app.contentRowTextFieldValue(penultimateIdx)
        let lastTitle = try app.contentRowTextFieldValue(tailIdx)
    ...
    func test_300_anItemsPriorityInTheWaitingListCanBeIncreasedByTheNudgeUpShortcut() throws {
        ...
        let itemOneTitle = try app.contentRowTextFieldValue(itemOneIdx)
        let itemTwoTitle = try app.contentRowTextFieldValue(itemTwoIdx)
    ...
    func test_400_anItemsPriorityInTheWaitingListCanBeDecreasedByTheNudgeDownShortcut() throws {
        ...
        try app.contentRowTextField(0).click() // stop it jumping end of the Sidebar List
        app.shortcutSelectEndOfList()
        ...
        XCTAssertEqual(try app.contentRowTextFieldValue(itemTwoIdx + 2), itemTwoTitle,
                       "And the second should as well")

        // Now move them down to what should be the tail of the list
        app.shortcutSelectedItemsMoveDownInList()
        app.shortcutSelectedItemsMoveDownInList()
        XCTAssertEqual(try app.contentRowTextFieldValue(try app.contentRows().count - 2), itemOneTitle,
                       "And nudging them up twice more should put the first of the pair second from last")
        XCTAssertEqual(try app.contentRowTextFieldValue(try app.contentRows().count - 1), itemTwoTitle,
    ...
    func test_700_changingPrioritiesInTheWaitingListIsUndoableAndRedoable() throws {
       try  app.sidebarWaitingList().click()
        ...
        XCTAssertEqual(try app.contentRowTextFieldValue(itemOneIdx - 2 + 1), itemTwoTitle,
                       "And the 6th in 3rd ")

        // Undoing them back to the original location
        app.shortcutUndo()
        app.shortcutUndo()
        XCTAssertEqual(try app.contentRowTextFieldValue(itemOneIdx), itemOneTitle,
                       "Then undoing both nudges, puts the original 4th item back in its original location")
        XCTAssertEqual(try app.contentRowTextFieldValue(itemTwoIdx), itemTwoTitle,
